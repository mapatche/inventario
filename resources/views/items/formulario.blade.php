...
@endrole
  ...
  <div>
    <label class="block text-sm font-semibold text-gray-700 mb-1">Imagen de factura</label>
    <input type="file" name="bill_location" accept="image/*" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
    @if (isset($item) && $item->bill_location)
        <div class="mt-2">
            <img src="{{ asset('storage/' . $item->bill_location) }}" alt="Factura" class="max-w-xs h-auto rounded-md">
        </div>
    @endif
  </div>
